...
**Research Question 2: How much does COVID-19 affect vehicle sales in 2020?**
...
names(brandsale)[6] <- "Sale2018"
names(brandsale)[7] <- "Sale2019"
names(brandsale)[8] <- "Sale2020"
...
gbs <- aggregate(brandsale[,3:8], list(brandsale$Month), FUN=sum)
mx <- t(as.matrix(gbs[-1]))
colors <- c("lightblue", "lightgreen", "lightpink", "grey", "lightyellow", "purple")

barplot(mx,main='Sales By Month', names = c(1:12), ylab='Sales', xlab='Month',border=F, beside = TRUE, col=colors, ylim=c(0,max(mx)*1.3))
# add a legend
box()
legend('topright',fill=colors,legend=c('2015Sales','2016Sales', '2017Sales','2018Sales','2019Sales','2020Sales'))
```
...
brandsale_test <-
  brandsale %>%
  select(Manufacturer, Sale2020, Sale2019, Sale2018)
...
brandsale_test <-
  brandsale_test %>%
  gather(key = "Year", value = "production", 2:4)
```
...
```{r}
result$Country <- "all"
result$Country[result$Manufacturer %in% c("Toyota","Mazda","Honda","Subaru","Nissan")] <- "Japan"
...
```{r}
fit.lm2 <- lm(Real.World.CO2 ~ Country + Year + production + Weight + Footprint + Horsepower + Fuel.Delivery.GDI + Engine.Displacement, data = result)
